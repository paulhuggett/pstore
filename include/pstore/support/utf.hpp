...
namespace pstore {
  namespace utf {
    ...
    private:
      ...
      static auto decode (gsl::not_null<std::uint8_t *> state, gsl::not_null<char32_t *> codep,
                          std::uint32_t byte) noexcept -> std::uint8_t;
    ...
    constexpr auto nop_swapper (std::uint16_t const v) noexcept -> std::uint16_t {
      return v;
    }
...
namespace pstore {
  namespace utf {
    ...
    template <typename SpanType>
    auto index (SpanType const span, std::size_t const pos) -> typename SpanType::element_type * {
