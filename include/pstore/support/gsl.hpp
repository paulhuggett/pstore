  ...
  namespace gsl {
    ...
    public:
      ...
      span<element_type, Count> subspan () const {
        PSTORE_ASSERT ((Offset == 0 || (Offset > 0 && Offset <= size ())) &&
                       (Count == dynamic_extent || (Count >= 0 && Offset + Count <= size ())));
        return {data () + Offset, Count == dynamic_extent ? size () - Offset : Count};
      }

      span<element_type, dynamic_extent>
      subspan (index_type const offset, index_type const count = dynamic_extent) const noexcept {
        PSTORE_ASSERT ((offset == 0 || (offset > 0 && offset <= size ())) &&
                       (count == dynamic_extent || (count >= 0 && offset + count <= size ())));
        return {data () + offset, count == dynamic_extent ? size () - offset : count};
      }
      ...
      constexpr reference operator() (index_type const idx) const { return this->operator[] (idx); }
