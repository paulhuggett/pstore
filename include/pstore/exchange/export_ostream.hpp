...
namespace pstore::exchange::export_ns {
  ...
  namespace details {
    ...
    template <typename T>
    using enable_if_unsigned = typename std::enable_if_t<std::is_unsigned_v<T>>;
    ...
    to_characters (Unsigned v, gsl::not_null<base10storage<Unsigned> *> const out) noexcept {
  ...
  ostream_base & ostream_base::write (gsl::span<char const, Extent> const s) {
    ...
    while (remaining > 0U) {
      ...
      PSTORE_ASSERT (count <= unsigned_cast (std::numeric_limits<span_index_type>::max ()));
