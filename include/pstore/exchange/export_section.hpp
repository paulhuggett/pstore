      ...
      namespace details {
        ...
        struct section_content_exporter<repo::generic_section> {
          ...
          OStream & operator() (OStream & os, indent const ind, class database const & db,
                                ...
                                repo::generic_section const & content, bool const comments) {
            return emit_object (
              os, ind, content,
              [&db, &strings, comments] (OStream & os1, indent const ind1,
                ...
                {
                  repo::container<repo::internal_fixup> const ifx = content1.ifixups ();
                  if (!ifx.empty ()) {
                    os1 << ",\n" << ind1 << R"("ifixups":)";
                ...
                {
                  repo::container<repo::external_fixup> const xfx = content1.xfixups ();
                  if (!xfx.empty ()) {
                    os1 << ",\n" << ind1 << R"("xfixups":)";
        ...
        struct section_content_exporter<repo::bss_section> {
          ...
          OStream & operator() (OStream & os, indent const ind, class database const & /*db*/,
                                ...
                                bool const /*comments*/) {
            return emit_object (
              os, ind, content, [] (OStream & os1, indent const ind1, bsss const & content1) {
                auto const * separator = "";
                {
                  auto const align = content1.align ();
                  if (align != 1U) {
