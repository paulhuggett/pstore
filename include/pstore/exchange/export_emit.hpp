...
namespace pstore {
  ...
  namespace exchange::export_ns {
    ...
    ostream_base & show_string (ostream_base & os, pstore::database const & db,
                                pstore::typed_address<pstore::indirect_string> addr, bool comments);
    ...
    OStream & emit_array_with_name (OStream & os, indent const ind, database const & db,
                                    InputIt const first, InputIt const last, bool const comments,
                                    Function const fn) {
      auto const * sep = "\n";
      os << "[";
  ...
  } // end namespace exchange::export_ns
