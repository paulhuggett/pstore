...
namespace pstore::exchange::import_ns {
  ...
  public:
    address_patch (gsl::not_null<database *> db, extent<repo::fragment> const & ex) noexcept;
  ...
  public:
    fragment_sections (not_null<context *> ctxt, not_null<transaction_base *> transaction,
  ...
  private:
    ...
    struct section_importer_creator {
      std::error_code operator() (fragment_sections * const fs) const {
        using importer = section_to_importer_t<repo::enum_to_section_t<Kind>, decltype (oit_)>;
        return push_object_rule<importer> (fs, Kind, fs->names_, fs->section_contents (Kind),
                                           &fs->oit_);
      }
    };
    ...
    repo::section_content * section_contents (repo::section_kind const kind) noexcept {
      return &contents_[static_cast<std::underlying_type_t<repo::section_kind>> (kind)];
    }
