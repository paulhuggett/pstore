...
namespace pstore::exchange::import_ns {
  ...
  error_or<repo::section_content *> generic_section<OutputIterator>::content_object () {
    ...
    if (!from_base64 (std::begin (data_), std::end (data_), std::back_inserter (content_->data))) {
  ...
  std::error_code generic_section<OutputIterator>::end_object () {
    ...
    *out_ = std::make_unique<repo::section_to_creation_dispatcher<repo::generic_section>::type> (
      kind_, c.get ());
    return pop ();
  }
