...
namespace pstore::exchange::import_ns {
  ...
  std::error_code linked_definition<OutputIterator>::end_object () {
    ...
    **out_ = repo::linked_definitions::value_type (*compilation, index,
                                                   typed_address<repo::definition>::make (index_));
    return pop ();
  }
  ...
  public:
    ...
    linked_definitions_section & operator= (linked_definitions_section const &) = delete;
    linked_definitions_section & operator= (linked_definitions_section &&) noexcept = delete;
  ...
  std::error_code linked_definitions_section<OutputIterator>::end_array () {
    ...
    *out_ =
      std::make_unique<repo::linked_definitions_creation_dispatcher> (data, data + ld_->size ());
    return pop ();
  }
