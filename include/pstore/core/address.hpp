...
namespace pstore {
  ...
  struct address {
    ...
    static constexpr address max () noexcept {
      return address{(value_type{max_segment} << offset_number_bits) | value_type{max_offset}};
    }
  ...
  static_assert (std::is_standard_layout_v<address>, "address is not standard layout");
...
namespace pstore {
  ...
  public:
    ...
    constexpr bool operator== (typed_address const & rhs) const noexcept { return a_ == rhs.a_; }
  ...
  template <typename T>
  inline typed_address<T> operator- (typed_address<T> const lhs, std::uint64_t const rhs) noexcept {
