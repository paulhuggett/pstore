...
namespace pstore {
  ...
  void deleter (T * const p) noexcept {
    using T8 = std::conditional_t<std::is_const_v<T>, std::uint8_t const, std::uint8_t>;
    delete[] (reinterpret_cast<T8 *> (p));
  }
...
#undef PSTORE_CLANG_PRAGMA
  ...
  public:
    ...
    explicit database (std::string const & path, access_mode am, bool access_tick_enabled = true);
    ...
    explicit database (std::shared_ptr<File> const & file, bool access_tick_enabled = true)
            : database (
                file, std::make_unique<system_page_size> (),
                region::get_factory (file, storage::full_region_size, storage::min_region_size),
                access_tick_enabled) {}
    ...
    static constexpr bool small_files_enabled () noexcept { return region::small_files_enabled (); }
    ...
    std::shared_ptr<T> getrw (typed_address<T> const addr, std::size_t const elements) {
      ...
      return std::static_pointer_cast<T> (this->getrw (addr.to_address (), sizeof (T) * elements));
    }
    ///@}
    ...
    /// For unit testing
    class storage const & storage () const noexcept { return storage_; }
    /// For unit testing
    class storage & storage () noexcept { return storage_; }
  ...
  private:
    ...
    void check_get_params (address const addr, std::size_t const size, bool const writable) const;
  ...
  auto database::getrou (typed_address<T> const addr, std::size_t const elements) const
    ...
    return unique_pointer_cast<T const> (this->getrou (addr.to_address (), sizeof (T) * elements));
  }
  ...
  auto database::get_footer_pos (File & file) -> typed_address<trailer> {
    ...
    if (h->a.signature1 != header::file_signature1 || h->a.signature2 != header::file_signature2) {
