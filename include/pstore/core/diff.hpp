...
namespace pstore {
  ...
  namespace diff_details {
    ...
    private:
      ...
      /// \result The output iterator to which results were written.
      template <typename OutputIterator>
      OutputIterator visit_node (index_pointer node, unsigned shifts, OutputIterator out) const;
    ...
    template <typename Index>
    template <typename Node, typename OutputIterator>
    OutputIterator traverser<Index>::visit_intermediate (index::details::index_pointer const node,
                                                         unsigned const shifts,
                                                         OutputIterator out) const {
      std::pair<std::shared_ptr<void const>, Node const *> const p = Node::get_node (db_, node);
      PSTORE_ASSERT (std::get<Node const *> (p) != nullptr);
      for (auto child : *std::get<Node const *> (p)) {
        if (this->is_new (node)) {
          out =
            this->visit_node (index_pointer{child}, shifts + index::details::hash_index_bits, out);
        }
      }
      return out;
    }
  ...
  OutputIterator diff (database const & db, Index const & index, revision_number const old,
    ...
    diff_details::traverser<Index> t{db, index,
