//===- include/pstore/http/ws_server.hpp ------------------*- mode: C++ -*-===//
//*                                               *
...
namespace pstore::http {
  ...
  template <>
  inline frame_fixed_layout host_to_network<frame_fixed_layout> (frame_fixed_layout x) noexcept {
  ...
  namespace details {
    ...
    decode_payload (std::uint64_t expected_length, gsl::span<std::uint8_t const> const & mask,
  ...
  error_or_n<IO, frame> read_frame (Reader & reader, IO io) {
    ...
    return reader.get_span (io, gsl::make_span (&res, 1)) >>=
       [&reader] (IO io1, gsl::span<frame_fixed_layout> const & p1) {
  ...
  namespace details {
    ...
    error_or<IO> send_extended_length_message (Sender sender, IO io, frame_fixed_layout const & f,
  ...
  error_or<IO> close_message (Sender const sender, IO const io, frame const & wsp) {
    ...
    // specification."
    if (auto const payload_size = wsp.payload.size (); payload_size == 0U) {
      ...
      state = static_cast<close_status_code> (
        network_to_host (*reinterpret_cast<std::uint16_t const *> (wsp.payload.data ())));
  ...
  bool check_message_complete (Sender const sender, IO const io, frame const & wsp, opcode & op,
    ...
    if (wsp.fin) {
      ...
      if (log_received_messages) {
        std::string str;
        std::transform (std::begin (payload), std::end (payload), std::back_inserter (str),
