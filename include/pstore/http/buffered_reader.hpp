//===- include/pstore/http/buffered_reader.hpp ------------*- mode: C++ -*-===//
//*  _            __  __                   _                      _            *
...
namespace pstore::http {
  ...
  public:
    ...
    explicit buffered_reader (RefillFunction refill, std::size_t buffer_size = default_buffer_size);
    ...
    error_or_n<IO, gsl::span<typename SpanType::element_type>> get_span (IO io,
                                                                         SpanType const & sp);
    ...
    while (!available.empty () && !is_eof_) {
      if (pos_ == end_) {
        ...
        if (end == span_.begin ()) {
          ...
          is_eof_ = true;
          return error_or_n<IO, byte_span>{std::in_place, io,
  ...
  auto buffered_reader<IO, RefillFunction>::get_span (IO io, SpanType const & sp)
    ...
    auto cast = [] (IO io2, byte_span const & sp2) {
      ...
      return error_or_n<IO, gsl::span<element_type>>{std::in_place, io2,
                                                     gsl::make_span (first, first + elements)};
    };
  ...
  auto buffered_reader<IO, RefillFunction>::geto (IO io) -> geto_result_type {
    ...
    return get_span (io, gsl::make_span (&result, 1)) >>= [] (IO io2, byte_span const & sp) {
      return geto_result_type{std::in_place, io2,
  ...
  auto buffered_reader<IO, RefillFunction>::getc (IO io) -> getc_result_type {
    ...
    return geto (io) >>= [] (IO io2, maybe<std::uint8_t> const & mb) {
      ...
      return getc_result_type{std::in_place, io2, mc};
    };
  }
  ...
  auto buffered_reader<IO, RefillFunction>::gets_impl (IO io, std::string const & str)
    ...
    auto eos = [str] (IO io2) {
      ...
      // what we've got.
      return gets_result_type{std::in_place, io2,
    ...
    return this->getc (io) >>= [this, &str, eos] (IO io3, maybe<char> mc) {
      ...
      case cr:
        ...
        return this->getc (io3) >>= [this, str, eos, lf] (IO io4, maybe<char> mc2) {
          ...
          return gets_result_type{std::in_place, io4, just (str)};
        };
      case lf: return gets_result_type{std::in_place, io3, just (str)};
      default: break;
      }
  ...
  void buffered_reader<IO, RefillFunction>::check_invariants () noexcept {
    ...
    auto is_valid = [this] (gsl::span<std::uint8_t>::iterator it) noexcept {
      ...
      return p >= 0 && static_cast<std::make_unsigned_t<ptrdiff_t>> (p) <= buf_.size ();
    };
  ...
  make_buffered_reader (RefillFunction refiller, std::size_t buffer_size = default_buffer_size) {
