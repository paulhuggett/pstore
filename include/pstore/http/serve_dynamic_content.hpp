  ...
  namespace http {
    ...
    namespace details {
      ...
      struct commands_helper {
        ...
        using function_type = std::function<return_type (Sender, IO, query_container const &)>;
    ...
    error_or<IO> serve_dynamic_content (Sender sender, IO io, std::string uri) {
      ...
      auto it = pos != std::string::npos ? std::begin (uri) + details::clamp_to_signed_max (pos + 1)
                                         : std::end (uri);
      query_container arguments;
      if (it != std::end (uri)) {
        it = query_to_kvp (it, std::end (uri), pstore::http::make_insert_iterator (arguments));
      }
      ...
      auto const lb = std::lower_bound (
        ...
        value_type{command,
                   [] (Sender, IO io2, query_container const &) { return error_or<IO>{io2}; }},
        compare);
