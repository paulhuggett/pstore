//===- include/pstore/adt/arrayvec.hpp --------------------*- mode: C++ -*-===//
//*                                             *
...
//
//===----------------------------------------------------------------------===//
#ifndef PSTORE_ARRAYVEC_HPP
#define PSTORE_ARRAYVEC_HPP
...
namespace pstore {
...
#else
  template <typename To, typename From,
            typename = typename std::enable_if_t<std::is_trivial_v<To> && std::is_trivial_v<From>>>
#endif
  constexpr To pointer_cast (From p) noexcept {
#if PSTORE_CPP_LIB_BIT_CAST
  ...
  public:
    ...
    PSTORE_CXX20REQUIRES ((std::forward_iterator<ForwardIterator>) )
    ...
    /// \name Element access
    ///@{
    constexpr T const * data () const noexcept { return pointer_cast<T const *> (data_.data ()); }
    ...
    PSTORE_CXX20REQUIRES ((std::input_iterator<InputIterator>) )
  ...
  private:
    ...
    std::array<typename std::aligned_storage_t<sizeof (T), alignof (T)>, Size> data_;
  };
  ...
  public:
    ...
    constexpr checked_back_insert_iterator &
