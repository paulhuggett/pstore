...
namespace pstore {
  ...
  namespace details {
    ...
    constexpr decltype (auto) get_accessor () noexcept {
      return
        [] (Iterator it)
          -> std::tuple_element_t<I, typename std::iterator_traits<Iterator>::value_type> const & {
          return std::get<I> (*it);
        };
    }
  ...
  public:
    ...
    template <typename IteratorIdx, typename IteratorV>
    static auto make_unique (IteratorIdx first_index, IteratorIdx last_index, IteratorV first_value,
                             IteratorV last_value) -> std::unique_ptr<sparse_array>;
    ...
    reference operator[] (size_type pos) noexcept { return sparse_array::index_impl (*this, pos); }
    ...
    constexpr std::size_t size_bytes () const { return sparse_array::size_bytes (this->size ()); }
  ...
  private:
    ...
    void * operator new (std::size_t count, InputIterator indices_first,
      ...
      return ::operator new (sparse_array::required_bytes (count, indices_first, indices_last));
    }
  ...
  sparse_array<ValueType, BitmapType>::sparse_array (IteratorIdx first_index,
                                                     IteratorIdx last_index, IteratorV first_value,
                                                     IteratorV last_value)
  ...
  template <typename IteratorIdx, typename IteratorV>
  auto
  sparse_array<ValueType, BitmapType>::make_unique (IteratorIdx first_index, IteratorIdx last_index,
                                                    IteratorV first_value, IteratorV last_value)
    -> std::unique_ptr<sparse_array> {

    return std::unique_ptr<sparse_array<ValueType, BitmapType>>{
      new (first_index, last_index)
        sparse_array<ValueType, BitmapType> (first_index, last_index, first_value, last_value)};
  }
  ...
  BitmapType sparse_array<ValueType, BitmapType>::bitmap (InputIterator first, InputIterator last) {
    return std::accumulate (
      ...
      [] (BitmapType const mm, typename std::iterator_traits<InputIterator>::value_type const idx) {
        ...
        PSTORE_ASSERT ((mm & mask) == 0U && "The same index must not appear more than once in the "
