//===- include/pstore/os/file.hpp -------------------------*- mode: C++ -*-===//
//*   __ _ _       *
//*  / _(_) | ___  *
//* | |_| | |/ _ \ *
//* |  _| | |  __/ *
  ...
  namespace file {
    namespace details {
      ...
      std::string name_from_template (std::string const & tmpl, RandomGenerator rng) {
        ...
        auto const it =
          std::find_if (tmpl.rbegin (), tmpl.rend (), [] (char const c) { return c != 'X'; });
    ...
    public:
      ...
      bool lock (std::uint64_t const /*offset*/, std::size_t const /*size*/, lock_kind const /*lt*/,
    ...
    public:
      ...
      void open (temporary const t) { return open (t, file_handle::get_temporary_directory ()); }
...
#ifdef _WIN32
#else
      static int lock_reg (int fd, int cmd, short type, off_t offset, short whence, off_t len);
#endif
