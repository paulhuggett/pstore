...
namespace pstore::romfs {
  ...
  struct stat {
    ...
    constexpr stat (std::size_t const size_, mode_t const mode_, std::time_t const mtime_) noexcept
            : size{size_}
            , mode{mode_}
            , mtime{mtime_} {}
    ...
    constexpr bool operator!= (stat const & rhs) const noexcept { return !operator== (rhs); }
  ...
  public:
    ...
    constexpr bool is_directory () const noexcept { return stat_.mode == mode_t::directory; }

  private:
    gsl::not_null<gsl::czstring> const name_;
    gsl::not_null<void const *> const contents_;
    struct stat const stat_;
  };
