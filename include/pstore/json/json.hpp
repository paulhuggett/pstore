...
namespace pstore::json {
  ...
  public:
    ...
    explicit parser (Callbacks callbacks = Callbacks{}, extensions extensions = extensions::none);
  ...
  namespace details {
    ...
    constexpr bool isspace (char const c) noexcept {
      return c == char_set::tab || c == char_set::lf || c == char_set::cr || c == char_set::space;
    }
    ...
    public:
      ...
      virtual std::pair<pointer, bool> consume (parser<Callbacks> & parser, maybe<char> ch) = 0;
    ...
    public:
      ...
      std::pair<typename matcher<Callbacks>::pointer, bool> consume (parser<Callbacks> & parser,
                                                                     maybe<char> ch) override;
    ...
    token_matcher<Callbacks, DoneFunction>::consume (parser<Callbacks> & parser, maybe<char> ch) {
      bool match = true;
    ...
    // ~~~~~~~~~~~~~~~~~~~
    template <typename Callbacks>
    bool number_matcher<Callbacks>::do_leading_minus_state (parser<Callbacks> & parser, char c) {
      bool match = true;
      if (c == '-') {
    ...
    // ~~~~~~~~~~
    template <typename Callbacks>
    bool number_matcher<Callbacks>::do_frac_state (parser<Callbacks> & parser, char const c) {
      bool match = true;
      if (c == '.') {
    ...
    // ~~~~~~~~~~
    template <typename Callbacks>
    bool number_matcher<Callbacks>::do_frac_digit_state (parser<Callbacks> & parser, char const c) {
    ...
    // ~~~~~~~~~~~~~~~~~~~
    template <typename Callbacks>
    bool number_matcher<Callbacks>::do_exponent_sign_state (parser<Callbacks> & parser, char c) {
      bool match = true;
      this->number_is_float ();
    ...
    // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    template <typename Callbacks>
    bool number_matcher<Callbacks>::do_integer_initial_digit_state (parser<Callbacks> & parser,
                                                                    char const c) {
    ...
    public:
      explicit string_matcher (gsl::not_null<std::string *> const str, bool object_key) noexcept
              : matcher<Callbacks> (start_state)
    ...
    private:
      ...
      static maybe<std::tuple<unsigned, state>> consume_hex_state (unsigned hex, enum state state,
                                                                   char32_t code_point);
    ...
    bool string_matcher<Callbacks>::appender::append16 (char16_t const cu) {
      ...
      if (utf::is_utf16_high_surrogate (cu)) {
        ...
        if (!this->has_high_surrogate ()) {
          ...
          std::tie (first, code_point) = utf::utf16_to_code_point (first, last, utf::nop_swapper);
    ...
    // ~~~~~~~~~
    template <typename Callbacks>
    maybe<unsigned> string_matcher<Callbacks>::hex_value (char32_t const c, unsigned const value) {
      auto digit = 0U;
      if (c >= '0' && c <= '9') {
    ...
    // ~~~~~~~~~~~~~~~~~
    template <typename Callbacks>
    auto string_matcher<Callbacks>::consume_hex_state (unsigned const hex, enum state const state,
    ...
    string_matcher<Callbacks>::consume (parser<Callbacks> & parser, maybe<char> ch) {
      ...
      if (maybe<char32_t> const code_point = decoder_.get (static_cast<std::uint8_t> (*ch))) {
        switch (this->get_state ()) {
        ...
        case start_state:
          ...
          break;
        case normal_char_state: {
          auto const normal_resl = string_matcher::consume_normal_state (parser, *code_point, app_);
          this->set_state (std::get<0> (normal_resl));
          this->set_error (parser, std::get<std::error_code> (normal_resl));
        } break;

        case escape_state: {
          auto const escape_resl = string_matcher::consume_escape_state (*code_point, app_);
        ...
        case hex3_state:
        case hex4_state: {
          maybe<std::tuple<unsigned, state>> const hex_resl = string_matcher::consume_hex_state (
    ...
    array_matcher<Callbacks>::consume (parser<Callbacks> & parser, maybe<char> ch) {
      ...
      case comma_state:
        ...
        switch (c) {
        case ',':
          this->set_state ((parser.extension_enabled (extensions::array_trailing_comma))
                             ? first_object_state
                             : object_state);
          return {this->make_whitespace_matcher (parser), true};
    ...
    object_matcher<Callbacks>::consume (parser<Callbacks> & parser, maybe<char> ch) {
      ...
      case comma_state:
        ...
        if (c == ',') {
          ...
          this->set_state ((parser.extension_enabled (extensions::object_trailing_comma))
                             ? first_key_state
                             : key_state);
    ...
    whitespace_matcher<Callbacks>::consume (parser<Callbacks> & parser, maybe<char> ch) {
      if (!ch) {
        ...
        case multi_line_comment_ending_state:
          ...
          this->set_state (c == details::char_set::slash ? body_state
                                                         : multi_line_comment_body_state);
          break;
        ...
        case single_line_comment_state:
          PSTORE_ASSERT (parser.extension_enabled (extensions::bash_comments) ||
    ...
    template <typename Callbacks>
    std::pair<typename matcher<Callbacks>::pointer, bool>
    whitespace_matcher<Callbacks>::consume_comment_start (parser<Callbacks> & parser, char c) {
      using details::char_set;
      if (c == char_set::slash && parser.extension_enabled (extensions::single_line_comments)) {
        this->set_state (single_line_comment_state);
    ...
    template <typename Callbacks>
    std::pair<typename matcher<Callbacks>::pointer, bool>
    whitespace_matcher<Callbacks>::multi_line_comment_body (parser<Callbacks> & parser, char c) {
      using details::char_set;
      PSTORE_ASSERT (parser.extension_enabled (extensions::multi_line_comments));
      PSTORE_ASSERT (this->get_state () == multi_line_comment_body_state);
      switch (c) {
    ...
    root_matcher<Callbacks>::consume (parser<Callbacks> & parser, maybe<char> ch) {
      ...
      case new_token_state: {
        ...
        case '8':
        case '9':
          return {this->template make_terminal_matcher<number_matcher<Callbacks>> (parser), false};
        case '"':
        ...
        case 't':
          return {
            this->template make_terminal_matcher<true_token_matcher<Callbacks>> (parser, "true"),
            false};
        case 'f':
          return {
            this->template make_terminal_matcher<false_token_matcher<Callbacks>> (parser, "false"),
            false};
        case 'n':
          return {
            this->template make_terminal_matcher<null_token_matcher<Callbacks>> (parser, "null"),
            false};
        case '[':
          return {typename matcher<Callbacks>::pointer (new array_matcher<Callbacks> ()), false};
        case '{':
          return {typename matcher<Callbacks>::pointer (new object_matcher<Callbacks> ()), false};
        default: this->set_error (parser, error_code::expected_token); return {nullptr, true};
        }
      } break;
      case done_state: PSTORE_ASSERT (false); break;
      }
  ...
  parser<Callbacks>::parser (Callbacks callbacks, extensions const extensions)
    ...
    stack_.push (
      mpointer (new (&singletons_->eof) details::eof_matcher<Callbacks>{}, deleter{false}));
    // We permit whitespace after the top-level object.
    stack_.push (mpointer (new (&singletons_->trailing_ws) details::whitespace_matcher<Callbacks>{},
                           deleter{false}));
