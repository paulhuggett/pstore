...
namespace {
  ...
  auto build_sections (Iterator begin, Iterator end)
    ...
    std::for_each (begin, end, [&dispatchers] (section_content const & section) {
      dispatchers.emplace_back (new generic_section_creation_dispatcher (section.kind, &section));
    });
    return dispatchers;
  }
  ...
  pstore::extent<fragment> build_fragment (transaction & transaction, Iterator begin,
                                           Iterator end) {
    std::vector<std::unique_ptr<section_creation_dispatcher>> fdata = build_sections (begin, end);
...
TEST_F (FragmentTest, MakeTextSectionWithFixups) {
  ...
  EXPECT_THAT (
    s.xfixups (),
    ElementsAre (
      external_fixup{indirect_string_address (3), relocation_type{3}, pstore::repo::binding::strong,
                     UINT64_C (3) /*offset*/, INT64_C (3) /*addend*/},
      external_fixup{indirect_string_address (4), relocation_type{4}, pstore::repo::binding::weak,
                     UINT64_C (4) /*offset*/, INT64_C (4) /*addend*/},
      external_fixup{indirect_string_address (5), relocation_type{5}, pstore::repo::binding::strong,
                     UINT64_C (5) /*offset*/, INT64_C (5) /*addend*/}));
}
...
TEST_F (FragmentTest, MakeTextSectionWithLinkedDefinitions) {
  ...
  auto extent =
    build_fragment (transaction_, std::begin (c), std::end (c), d.data (), d.data () + d.size ());
