...
TEST_F (ExchangeSectionFixups, RoundTripInternalCollection) {
  ...
  ifixups.emplace_back (pstore::repo::section_kind::thread_data, pstore::repo::relocation_type{41},
                        std::uint64_t{43} /*offset */, std::int64_t{47} /*addend*/);
...
TEST_F (ExternalFixupMembersImport, Name) {
  ...
  {
    auto const & parser2 = this->parse (R"({ "name":"name", "type":13, "offset":19, "addend":23 })",
                                        &db_, imported_names);
    ASSERT_TRUE (parser2.has_error ()) << "Expected the parse to fail";
    EXPECT_EQ (parser2.last_error (),
  ...
  {
    auto const & parser3 =
      this->parse (R"({ "name":1, "type":13, "offset":19, "addend":23 })", &db_, imported_names);
...
TEST_F (ExternalFixupMembersImport, Type) {
  ...
  {
    auto const & parser2 =
      this->parse (R"({ "name":0, "type":true, "offset":19, "addend":23 })", &db_, imported_names);
    ASSERT_TRUE (parser2.has_error ()) << "Expected the parse to fail";
    EXPECT_EQ (parser2.last_error (),
               make_error_code (pstore::exchange::import_ns::error::unexpected_boolean));
  }
...
TEST_F (ExternalFixupMembersImport, IsWeak) {
  ...
  {
    auto const & parser1 =
      this->parse (R"({ "name":0, "type":17, "offset":19, "addend":23 })", &db_, imported_names);
    EXPECT_FALSE (parser1.has_error ()) << "JSON error was: " << parser1.last_error ().message ();
  }
  // The is_weak key has the wrong type.
  {
    auto const & parser2 = this->parse (
      R"({ "name":0, "type":true, "is_weak":0, offset":19, "addend":23 })", &db_, imported_names);
    ASSERT_TRUE (parser2.has_error ()) << "Expected the parse to fail";
    EXPECT_EQ (parser2.last_error (),
               make_error_code (pstore::exchange::import_ns::error::unexpected_boolean));
  }
...
TEST_F (ExternalFixupMembersImport, Offset) {
  ...
  {
    auto const & parser2 =
      this->parse (R"({ "name":0, "type":17, "offset":true, "addend":23 })", &db_, imported_names);
    EXPECT_TRUE (parser2.has_error ()) << "Expected the parse to fail";
    EXPECT_EQ (parser2.last_error (),
               make_error_code (pstore::exchange::import_ns::error::unexpected_boolean));
  }
...
TEST_F (ExternalFixupMembersImport, Addend) {
  ...
  {
    auto const parser2 =
      this->parse (R"({ "name":0, "type":17, "offset":19, "addend":true })", &db_, imported_names);
    EXPECT_TRUE (parser2.has_error ()) << "Expected the parse to fail";
    EXPECT_EQ (parser2.last_error (),
               make_error_code (pstore::exchange::import_ns::error::unexpected_boolean));
  }
