...
namespace {
  ...
  decltype (auto)
  import_compilation_parser (transaction * const transaction,
                             pstore::exchange::import_ns::string_mapping * const names,
    ...
    return make_json_object_parser<pstore::exchange::import_ns::compilation> (
      &transaction->db (), transaction, names, std::cref (fragment_index), std::cref (digest));
  }
  ...
  pstore::extent<pstore::repo::fragment> build_fragment (transaction & transaction,
    ...
    pstore::extent<pstore::repo::fragment> const fext =
      pstore::repo::fragment::alloc (transaction, std::begin (dispatcher), std::end (dispatcher));
...
TEST_F (ExchangeCompilation, Empty) {
  ...
  export_ns::ostringstream exported_compilation_stream;
  {
    ...
    pstore::extent<pstore::repo::compilation> const compilation = pstore::repo::compilation::alloc (
      transaction, indir_strings[triple], std::begin (definitions), std::end (definitions));
  ...
  ASSERT_EQ (imported_compilation->size (), 0U) << "The compilation should contain no definitions";
}
