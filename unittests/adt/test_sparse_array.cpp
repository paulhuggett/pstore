...
TYPED_TEST (SparseArray, IndexInitializationList) {
  ...
  EXPECT_EQ (pstore::get<2> (*arr), empty);
}
...
TYPED_TEST (SparseArray, PairInitialization) {
  std::vector<std::pair<std::size_t, char const *>> const src{{0, "zero"}, {2, "two"}, {4, "four"}};
  auto arrp = sparse_array<std::string, TypeParam>::make_unique (std::begin (src), std::end (src));
  auto & arr = *arrp;
...
TYPED_TEST (SparseArray, SizeBytesAgree) {
  ...
  EXPECT_EQ ((sparse_array<unsigned, TypeParam>::make_unique (std::begin (empty), std::end (empty))
                ->size_bytes ()),
