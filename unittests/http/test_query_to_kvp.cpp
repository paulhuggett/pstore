...
TEST (QueryToKvp, EmptyString) {
  string_map result;
  query_to_kvp ("", pstore::http::make_insert_iterator (result));
  EXPECT_THAT (result, ContainerEq (string_map{}));
}

TEST (QueryToKvp, SingleKvp) {
  string_map result;
  query_to_kvp ("key=value", pstore::http::make_insert_iterator (result));
  EXPECT_THAT (result, ContainerEq (string_map{{"key", "value"}}));
}

TEST (QueryToKvp, TwoKvps) {
  string_map result;
  query_to_kvp ("a=1&b=2", pstore::http::make_insert_iterator (result));
...
TEST (QueryToKvp, BadQueryStringValue) {
  string_map result;
  query_to_kvp ("param1=hello=world&param2=false", pstore::http::make_insert_iterator (result));
  EXPECT_THAT (result, ContainerEq (string_map{{"param1", "hello=world"}, {"param2", "false"}}));
}

TEST (QueryToKvp, MissingValue) {
  string_map result;
  query_to_kvp ("param1=&param2=false", pstore::http::make_insert_iterator (result));
  EXPECT_THAT (result, ContainerEq (string_map{{"param1", ""}, {"param2", "false"}}));
}

TEST (QueryToKvp, DuplicateKeyIgnored) {
  string_map result;
  query_to_kvp ("k1=v1&k1=v2", pstore::http::make_insert_iterator (result));
...
TEST (QueryToKvp, Plus) {
  string_map result1;
  query_to_kvp ("k1=v1+v2", pstore::http::make_insert_iterator (result1));
...
TEST (QueryToKvp, Hex) {
  {
    // Hex digits
    string_map result1;
    query_to_kvp ("k1=v1%20v2", pstore::http::make_insert_iterator (result1));
    EXPECT_THAT (result1, ContainerEq (string_map{{"k1", "v1 v2"}}));
  }
  {
    // Mixed upper/lower hex letters
    string_map result2;
    query_to_kvp ("k1=v1%C2%a9v2", pstore::http::make_insert_iterator (result2));
    EXPECT_THAT (result2, ContainerEq (string_map{{"k1", "v1\xc2\xa9v2"}}));
  }
  {
    // Partial hex value.
    string_map result3;
    query_to_kvp ("k1=v1%Cv2", pstore::http::make_insert_iterator (result3));
    EXPECT_THAT (result3, ContainerEq (string_map{{"k1", "v1\x0cv2"}}));
  }
  {
    // Trailing percent.
    string_map result4;
    query_to_kvp ("k1=v1%", pstore::http::make_insert_iterator (result4));
...
TEST (KvpToQuery, Empty) {
  string_map const in;
  std::string out;
  pstore::http::kvp_to_query (std::begin (in), std::end (in), std::back_inserter (out));
  EXPECT_EQ (out, "");
}
