...
TEST (StatusLine, SwitchingProtocols) {
  EXPECT_EQ (
    pstore::http::build_status_line (pstore::http::http_status_code::switching_protocols, "path"),
    "HTTP/1.1 101 path\r\n");
}

TEST (StatusLine, BadRequest) {
  EXPECT_EQ (pstore::http::build_status_line (pstore::http::http_status_code::bad_request, "path"),
             "HTTP/1.1 400 path\r\n");
}
...
TEST (StatusLine, InternalServerError) {
  EXPECT_EQ (
    pstore::http::build_status_line (pstore::http::http_status_code::internal_server_error, "path"),
    "HTTP/1.1 500 path\r\n");
}
