...
TEST (MCRepoSectionContent, Eq) {
  auto const populate = [] (section_content * const sc) {
    ...
    sc->ifixups.clear ();
    std::fill_n (
      std::back_inserter (sc->ifixups), 3U,
      internal_fixup (section_kind::read_only, relocation_type{1}, 5U /*offset*/, 13U /*addend*/));
    // TODO: xfixups.
  };
...
TEST_F (MCRepoFixture, DumpFragment) {
  ...
  std::array<linked_definitions::value_type, 1> linked_definitions{{linked_definitions::value_type{
...
TEST_F (MCRepoFixture, DumpCompilation) {
  ...
  EXPECT_THAT (split_tokens (lines.at (line++)), ElementsAre ("triple", ":", "machine-vendor-os"));
}
...
TEST_F (MCRepoFixture, DumpBssSection) {
  ...
  std::shared_ptr<fragment const> frag = [this] () {
    ...
    std::array<section_content, 1> c = {{{section_kind::bss, std::uint8_t{0x10} /*alignment*/}}};
