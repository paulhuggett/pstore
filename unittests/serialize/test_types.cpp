...
namespace pstore::serialize {
  ...
  // Note that read() and write() are not implemented. This ensures that if the
  // code under test tries to call either of them, we'll get a compilation failure.
  template <>
...
namespace {
  ...
  protected:
    ...
    public:
      ...
      result_type putn (pstore::gsl::span<ElementType, Extent> span) {
        this->putn_mock (span);
        return {};
      }

      template <typename ElementType, std::ptrdiff_t Extent>
      void getn (pstore::gsl::span<ElementType, Extent> span) {
        this->getn_mock (span);
      }
    };
  };
...
TEST_F (ArchiveSpan, WriteSpan) {
  ...
  pstore::serialize::write (archive, pstore::gsl::make_span (arr));
}
...
TEST_F (Flood, Four) {
  ...
  EXPECT_THAT (
    buffer_, testing::ElementsAre (expected_[0], expected_[1], expected_[2], expected_[3], zero_));
}
// NOLINTNEXTLINE
TEST_F (Flood, Five) {
