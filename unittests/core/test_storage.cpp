...
TEST_F (RequestSpansRegions, FullRegionSize) {
...

  {
    ...
    EXPECT_TRUE (st1.request_spans_regions (pstore::address{min_region_size} - 1U, std::size_t{2}));
  }
  {
    pstore::database db2{file};
    db2.set_vacuum_mode (pstore::database::vacuum_mode::disabled);
    ...
    EXPECT_FALSE (st2.request_spans_regions (pstore::address{full_region_size}, std::size_t{1}));
    EXPECT_TRUE (
