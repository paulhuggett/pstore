...
TEST_F (Database, CheckInitialState) {
...

  {
    ...
    auto const expected =
      std::array<std::uint16_t, 2>{{pstore::header::major_version, pstore::header::minor_version}};
  ...
  }
  {
    ...
    EXPECT_THAT (pstore::trailer::default_signature1, testing::ContainerEq (footer->a.signature1));
    ...
    EXPECT_THAT (pstore::trailer::default_signature2, testing::ContainerEq (footer->signature2));
  }
}
...
TEST_F (Database, Read16Bytes) {
  ...
  pstore::address addr = db.allocate (size, align);
  {
    ...
    std::shared_ptr<std::uint8_t> w = db.getrw (pstore::typed_address<std::uint8_t> (addr), size);
    auto * const p = w.get ();
...
TEST_F (OpenCorruptStore, HeaderFooterTooLarge) {
  auto too_large = pstore::typed_address<pstore::trailer>::make (
    (UINT64_C (1) << (pstore::address::offset_number_bits + pstore::address::segment_number_bits)) -
    1U);
