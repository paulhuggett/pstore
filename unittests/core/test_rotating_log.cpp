...
TEST (RotatingLog, OneFile) {
  ...
  Expectation open = EXPECT_CALL (log.stream_traits (), open (_, StrEq ("base_name"), _)).Times (1);
  ...
  EXPECT_TRUE (log.is_open ()) << "Expected the log file to open after a message has been written";
