...
namespace {
  ...
  public:
    ...
    void add (pstore::gsl::not_null<std::vector<pstore::region::memory_mapper_ptr> *> regions,
      ...
      this->append<pstore::file::in_memory, mock_mapper> (file_, regions, original_size, new_size);
    }
...
TEST_F (EmptyStore, ProtectAllOfOneRegion) {
  ...
  if (pstore::database::small_files_enabled ()) {
    ...
    void * addr =
      reinterpret_cast<std::uint8_t *> (store_.file ()->data ().get ()) + fixed_page_size_bytes;
