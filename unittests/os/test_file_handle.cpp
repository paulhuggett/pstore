...
namespace {
  ...
  protected:
    struct mock_callback {
      ...
      std::size_t operator() (void const * p, std::size_t size) const { return call (p, size); }
    };
...
TYPED_TEST (SplitFixture, SplitUint8TwiceMaxPlus1) {
  ...
  auto buffer = std::make_unique<TypeParam[]> (std::numeric_limits<std::uint8_t>::max () * 2U + 1U);
