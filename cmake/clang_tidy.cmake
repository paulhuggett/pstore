...
function (pstore_find_clang_tidy tidy_path sys_includes)
  ...
  if (can_tidy)
    ...
    # If the PSTORE_SYS_INCLUDES cache variable has not been set by an earlier
    ...
    if (NOT PSTORE_SYS_INCLUDES)
      if (PSTORE_CLANG_TIDY STREQUAL "PSTORE_CLANG_TIDY-NOTFOUND")
        ...
        # Extract the default system include paths from the compiler. We run it
        # with '-v' and those are written to stderr along with other information
        # data doesn't interest us.
        execute_process (
          COMMAND g++ -E -x c++ - -v
          INPUT_FILE "/dev/null"
        ...
        # everything after "#include <...> search starts here:" and before "End
        # of search list.". I also remove any macOS framework directories.
        execute_process (
          COMMAND echo "${include_paths}"
          COMMAND sed -e "1,/> search starts here:/d" -e "s/ \\(.*\\)/\\1/" -e
                  "/of search list/,$d" -e "/\\(framework directory\\)/d"
        ...
        # Finally we get to build the system include directory switches. Each
        # path has leading and trailing whitespace stripped and an -isystem
        # switch preceeding it.
        ...
        endforeach (P)
        set (PSTORE_SYS_INCLUDES ${sys_includes}
             CACHE INTERNAL "The compiler's system include paths"
        )
      endif ()
...
function (add_clang_tidy_target source_target)
  ...
  if (NOT tidy_path STREQUAL "tidy_path-NOTFOUND")
    ...
    # In cmake 3.6 or later we could use list (FILTER source_files INCLUDE REGEX
    # "cpp") However, this needs to work in 3.4...
    ...
    foreach (src_file ${source_files})
      ...
      # Only target source files ending in .cpp and in the target's source
      # directory.
    ...
    add_custom_target (
      "${tidy_target}"
      COMMAND
        "${tidy_path}" -header-filter="${PROJECT_SOURCE_DIR}/include/.*"
        ${cpp_files} -- -std=c++14 -D NDEBUG
    ...
    set_target_properties (
      "${tidy_target}" PROPERTIES FOLDER "pstore clang-tidy"
    )
