...
# the sources and libraries in the locations given by the following two
# variables:

set (PSTORE_KLEE_SRC_DIR "/home/klee/klee_src/include")
find_library (
  PSTORE_KLEE_LIB_DIR libkleeRuntest.so PATHS /home/klee/klee_build/klee/lib
...
function (pstore_can_klee result)
  ...
  if (CMAKE_CXX_COMPILER_ID MATCHES "Clang$")
    ...
    if (NOT "${PSTORE_KLEE_PATH}" STREQUAL "PSTORE_KLEE_PATH-NOTFOUND")
      ...
      # The klee custom cmake targets require that we running cmake 3.9 or
      # later.
      if (${CMAKE_MAJOR_VERSION} GREATER 3)
        set (${result} Yes PARENT_SCOPE)
      elseif ((${CMAKE_MAJOR_VERSION} EQUAL 3) AND (${CMAKE_MINOR_VERSION}
                                                    GREATER_EQUAL 9)
      )
        set (${result} Yes PARENT_SCOPE)
      endif ()
...
function (pstore_configure_klee_test_target name)
  ...
  target_compile_options (
    ${name} PRIVATE -g -fno-threadsafe-statics -target x86_64-pc-linux-gnu
  )
  set_target_properties (
    ${name} PROPERTIES FOLDER "pstore-klee" CXX_STANDARD ${pstore_cxx_version}
...
function (pstore_add_klee_test)
  ...
  if (can_klee)
    ...
    foreach (dependent ${depends})
      # If the dependent target has public include directories, then add the
      # same to the bitcode library.
    ...
    set (link_llvm_lib "")
    foreach (dependent ${depends})
      list (APPEND link_llvm_lib
            "--link-llvm-lib=$<TARGET_FILE:${dependent}-bc>"
      )
    endforeach (dependent)
    list (APPEND link_llvm_lib
          --link-llvm-lib=$<TARGET_OBJECTS:pstore-klee-cxxstdlib-bc>
    )
    ...
    add_custom_target (
      ...
      COMMAND
        ...
        --optimize ${link_llvm_lib} $<TARGET_OBJECTS:${bc_tname}>
      COMMAND "${PSTORE_RUN_KLEE}" --ktest-tool $<TARGET_FILE:${exe_tname}>
              $<TARGET_OBJECTS:${bc_tname}>
      DEPENDS "${bc_tname}" ${exe_tname}
