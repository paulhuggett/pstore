...
function (pstore_standalone_compiler_setup)
  ...
  if (${arg_IS_UNIT_TEST})
    list (APPEND clang_options -Wno-unused-member-function
  ...
  # ~~~
  set (msvc_options)
  # Lots of warnings. Enable W4 but disable: 4324: structure was padded due to
  # alignment specifier
  list (APPEND msvc_options -W4 -wd4324)
...
function (pstore_add_additional_compiler_flags target_name)
  if (CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    ...
    # Disable the "unused lambda capture" warning since we must capture extra
  ...
  elseif (MSVC)
    ...
    # 2. unary minus applied to unsigned, result still unsigned.
...
# NAME - The name of the directory containing the targets include files. TARGET
# - The name of the target to be created. If omitted, defaults to the NAME value
# with a "pstore-" prefix. Underscores (_) are replaced with dash (-) to follow
# the pstore naming convention. SOURCES - A list of the library's source files.
# INCLUDES - A list of the library's include files.

function (add_pstore_library)
  cmake_parse_arguments (
    arg "" "TARGET;NAME;HEADER_DIR" "SOURCES;INCLUDES" ${ARGN}
  )
  ...
  # The collection of include file paths is built by stitching the HEADER_DIR on
  # to the front of each of the INCLUDES.
  set (include_files "")
  ...
  if (PSTORE_IS_INSIDE_LLVM)
    add_llvm_library (
      ${arg_TARGET} STATIC ${arg_SOURCES} ${include_files}
      ADDITIONAL_HEADER_DIRS "${arg_HEADER_DIR}"
    )
    ...
    set_target_properties (
      ${arg_TARGET}
      PROPERTIES CXX_STANDARD "${pstore_cxx_version}"
                 CXX_STANDARD_REQUIRED Yes
                 CXX_EXTENSIONS Off
                 # Full path of all of the public include files.
                 PUBLIC_HEADER "${include_files}"
                 # Produce PIC so that pstore static libraries can be used in a
                 # shared library.
    ...
    target_include_directories (
      ${arg_TARGET} PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}"
    )
  ...
  target_include_directories (
    ${arg_TARGET} PUBLIC $<BUILD_INTERFACE:${PSTORE_ROOT_DIR}/include>
...
function (add_pstore_executable target)
  ...
  # On MSVC, link setargv.obj to support the two wildcards (? and *) on the
  # command line.
  if (MSVC)
...
function (add_pstore_tool name)
  ...
  install (TARGETS ${name} EXPORT pstore
           RUNTIME DESTINATION "${CMAKE_INSTALL_BINDIR}" COMPONENT pstore
  )
...
function (add_pstore_example name)
  ...
  set_target_properties (
    example-${name} PROPERTIES EXCLUDE_FROM_ALL Yes FOLDER "pstore examples"
  )
