...
#
#===----------------------------------------------------------------------===//
function (pstore_split_version_string version_string version_major
...
function (pstore_find_nodejs node_path version_major version_minor
          version_patch
)
  ...
  find_program (
    nodejs_executable NAMES node nodejs DOC "Node.js executable location"
  )

  if (nodejs_executable)
    set (${node_path} "${nodejs_executable}" PARENT_SCOPE)

    # Figure out which version of Node.JS we've got. This will set
    # NODE_VERSION_STRING to the entire version string and
    # node_version_major/minor/patch accordingly.
    execute_process (
      COMMAND ${nodejs_executable} --version OUTPUT_VARIABLE _VERSION
      RESULT_VARIABLE node_version_result_
    )
    if (NOT node_version_result_)
      pstore_split_version_string (
        "${_VERSION}" version_major_ version_minor_ version_patch_
      )
      message (
        STATUS
          "Node.js was found at: ${nodejs_executable} (version ${version_major_}.${version_minor_}.${version_patch_})"
      )
      set (${version_major} ${version_major_} PARENT_SCOPE)
      set (${version_minor} ${version_minor_} PARENT_SCOPE)
      set (${version_patch} ${version_patch_} PARENT_SCOPE)
      ...
      message (
        STATUS "Node.js was found at: ${nodejs_executable} (unknown version)"
      )
    endif ()
...
function (pstore_find_npm npm_path version_major version_minor version_patch)
  ...
  find_program (npm_executable NAMES npm.cmd npm DOC "NPM executable location")
  if (npm_executable)
    set (${npm_path} "${npm_executable}" PARENT_SCOPE)

    # Figure out which version of NPM we've got.
    execute_process (
      COMMAND ${npm_executable} --version OUTPUT_VARIABLE version_
      RESULT_VARIABLE npm_version_result_
    )

    if (NOT npm_version_result_)
      pstore_split_version_string (
        "${version_}" version_major_ version_minor_ version_patch_
      )
      message (
        STATUS
          "NPM was found at: ${npm_executable} (version ${version_major_}.${version_minor_}.${version_patch_})"
      )
      set (${version_major} ${version_major_} PARENT_SCOPE)
      set (${version_minor} ${version_minor_} PARENT_SCOPE)
      set (${version_patch} ${version_patch_} PARENT_SCOPE)
      ...
      message (
        STATUS "NPM was found at: ${nodejs_executable} (unknown version)"
      )
    endif ()
  else (npm_executable)
