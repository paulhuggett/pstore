...
namespace {
  ...
  bool read_strings_index (pstore::database const & db, std::string const & key) {
    ...
    if (it == strings->cend (db)) {
      error_stream << pstore::utf::to_native_string (key) << PSTORE_NATIVE_TEXT (": not found")
                   << std::endl;
      // note that the program does not signal failure if the key is missing.
    } else {
      pstore::shared_sstring_view owner;
      out_stream << pstore::utf::to_native_string (it->as_db_string_view (&owner).to_string ());
    }
    return true;
  }
  ...
  bool read_names_index (pstore::database const & db, std::string const & key) {
    ...
    if (it == names->cend (db)) {
      ...
      while (size > 0) {
        ...
        size -=
          static_cast<std::make_unsigned<std::remove_const<decltype (size_to_write)>::type>::type> (
            size_to_write);
      }
    }
