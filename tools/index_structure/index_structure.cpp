...
namespace {
  ...
  std::string dump_intermediate (pstore::database const & db, IndexType const & index,
    ...
    for (auto const & child : *ptr) {
      auto const child_id = dump (db, index, os, index_pointer{child}, shifts + hash_index_bits);
      os << this_id << " -> " << child_id << ";\n";
    }
    return this_id;
  }
  ...
  template <typename IndexType>
  void dump_index (pstore::database const & db, IndexType const * const index, char const * name) {
    if (index == nullptr) {
