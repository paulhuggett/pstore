...
namespace pstore::index::details {
  ...
  linear_node::linear_node (std::size_t const size)
    ...
    static_assert (offsetof (linear_node, size_) == 8, "offsetof (linear_node, size_) must be 8");
    static_assert (offsetof (linear_node, leaves_) == 16, "offset of the first child must be 16");
  ...
  //*                              *
  branch::signature_type const branch::node_signature_ = {{'I', 'n', 't', 'e', 'r', 'n', 'a', 'l'}};
  ...
  branch::branch (index_pointer const & leaf, hash_type const hash)
    ...
    static_assert (std::is_standard_layout_v<branch>, "internal branch must be standard-layout");
    static_assert (alignof (linear_node) >= 4,
                   "branch must have alignment >= 4 to ensure the bottom two bits are 0");

    static_assert (offsetof (branch, signature_) == 0, "offsetof (branch, signature) must be 0");
  ...
  auto branch::read_node (database const & db, typed_address<branch> const addr)
    ...
    auto resl = std::static_pointer_cast<branch const> (db.getro (addr.to_address (), actual_size));
