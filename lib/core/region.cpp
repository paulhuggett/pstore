...
namespace pstore::region {
  ...
  void file_based_factory::add (gsl::not_null<std::vector<memory_mapper_ptr> *> const regions,
                                std::uint64_t const original_size, std::uint64_t const new_size) {
    this->append<file::file_handle, memory_mapper> (file_, regions, original_size, new_size);
  }
  ...
  mem_based_factory::mem_based_factory (std::shared_ptr<file::in_memory> file,
                                        std::uint64_t const full_size, std::uint64_t const min_size)
          : factory{full_size, min_size}
          , file_{std::move (file)} {}
  ...
  void mem_based_factory::add (gsl::not_null<std::vector<memory_mapper_ptr> *> const regions,
    ...
    this->append<file::in_memory, in_memory_mapper> (file_, regions, original_size, new_size);
  }
