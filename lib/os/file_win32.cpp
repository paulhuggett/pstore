...
namespace {
  ...
  HANDLE create_new_file (std::string const & path, bool is_temporary) {
    ...
    DWORD const flags =
      is_temporary ? (FILE_ATTRIBUTE_TEMPORARY | FILE_FLAG_DELETE_ON_CLOSE) : FILE_ATTRIBUTE_NORMAL;
  ...
  namespace file {
    ...
    void file_handle::open (create_mode create, writable_mode writable, present_mode present) {
      ...
      file_ = ::CreateFileW (pstore::utf::win32::to16 (path_).c_str (), // "file name"
                             desired_access, FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                             nullptr, // "security attributes"
                             creation_disposition,
      ...
      if (file_ == invalid_oshandle) {
        DWORD const last_error = ::GetLastError ();
        if (present == present_mode::allow_not_found && last_error == ERROR_FILE_NOT_FOUND) {
    ...
    void file_handle::write_buffer (gsl::not_null<void const *> buffer, std::size_t size) {
      ...
      if (bytes_written != size) {
        raise (std::errc::invalid_argument, "Didn't write the number of bytes that were requested");
      }
    ...
    std::string file_handle::get_temporary_directory () {
      ...
      if (num_code_units == 0) {
        ...
        num_code_units = ::GetTempPathW (static_cast<DWORD> (temp_path.size ()), temp_path.data ());
        if (num_code_units == 0) {
          DWORD const last_error = ::GetLastError ();
          raise (win32_erc{last_error}, "GetTempPathW");
        } else if (num_code_units != temp_path.size ()) {
