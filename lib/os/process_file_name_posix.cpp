...
namespace pstore {
  ...
  std::string process_file_name () {
    ...
    std::size_t const length = freebsd::process_file_name (gsl::make_span (mib), &::sysctl, buffer);
...
namespace pstore {
  ...
  std::string process_file_name () {
    ...
    auto read_link = [&path] (gsl::span<char> const buffer) {
      ...
      ssize_t const num_chars = ::readlink (
        path.c_str (), buffer.data (),
        clamp (static_cast<std::size_t> (buffer_size), std::size_t{0}, std::size_t{SSIZE_MAX}));
      if (num_chars < 0) {
