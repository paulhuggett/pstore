...
namespace {
  ...
  pstore::error_or<IO> send_bad_websocket_version (SenderFunction const sender, IO && io) {
    ...
    return sender (std::forward<IO> (io),
                   as_bytes (pstore::gsl::make_span (response_string))) >>= [&sender] (IO && io2) {
...
namespace pstore::http {
  ...
  void report_error (std::error_code const error, request_info const & request,
    ...
    if (cat == get_error_category ()) {
      ...
      case error_code::bad_request: return report (http_status_code::bad_request, "Bad request");
