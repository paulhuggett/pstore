  ...
  namespace http {
    ...
    auto sha1::result () noexcept -> result_type {
      ...
      for (auto i = 0U; i < hash_size; ++i) {
        digest[i] =
          static_cast<std::uint8_t> (intermediate_hash_[i >> 2U] >> 8U * (3U - (i & 0x03U)));
      }
      return digest;
    }
    ...
    void sha1::process_message_block () noexcept {
      ...
      for (auto t = 0U; t < 20U; ++t) {
        std::uint32_t const temp = circular_shift (5, a) + ((b & c) | ((~b) & d)) + e + w[t] + k[0];
        e = d;
        d = c;
    ...
    std::string source_key (std::string const & k) {
      ...
      static std::array<std::uint8_t const, 36> const magic = {{
        UINT8_C ('2'), UINT8_C ('5'), UINT8_C ('8'), UINT8_C ('E'), UINT8_C ('A'), UINT8_C ('F'),
        UINT8_C ('A'), UINT8_C ('5'), UINT8_C ('-'), UINT8_C ('E'), UINT8_C ('9'), UINT8_C ('1'),
        UINT8_C ('4'), UINT8_C ('-'), UINT8_C ('4'), UINT8_C ('7'), UINT8_C ('D'), UINT8_C ('A'),
        UINT8_C ('-'), UINT8_C ('9'), UINT8_C ('5'), UINT8_C ('C'), UINT8_C ('A'), UINT8_C ('-'),
        UINT8_C ('C'), UINT8_C ('5'), UINT8_C ('A'), UINT8_C ('B'), UINT8_C ('0'), UINT8_C ('D'),
        UINT8_C ('C'), UINT8_C ('8'), UINT8_C ('5'), UINT8_C ('B'), UINT8_C ('1'), UINT8_C ('1'),
      }};
