  ...
  upgrade_to_ws (Reader & reader, IO io, pstore::http::request_info const & request,
    ...
    auto const accept_ws_connection = [&header_contents, &io] () {
      ...
      return pstore::http::send (sender, io, status_line) >>= [&] (IO io2) {
        return pstore::http::send (
          sender, io2, pstore::http::build_headers (std::begin (headers), std::end (headers)));
      };
    };
    ...
    auto server_loop_thread = [&channels] (Reader && reader2, socket_descriptor io2,
      ...
      PSTORE_TRY {
        ...
        ws_server_loop (std::move (reader2), pstore::http::net::network_sender, std::ref (io2), uri,
                        channels);
...
#  else
    ...
    ssize_t const nread =
      recv (fd.native_handle (), buf.data (), static_cast<int> (buf.size ()), MSG_PEEK /*flags*/);
    int const err = errno;
    if (nread == -1) {
...
#  endif
  ...
  pstore::error_or<socket_descriptor> wait_for_connection (socket_descriptor const & parentfd) {
    ...
    socket_descriptor childfd{::accept (
      parentfd.native_handle (), reinterpret_cast<struct sockaddr *> (&client_addr), &clientlen)};
    if (!childfd.valid ()) {
