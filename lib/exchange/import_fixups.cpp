...
#undef X
  ...
  // name
  // ~~~~
  gsl::czstring internal_fixup::name () const noexcept {
    return "internal fixup";
  }
  ...
  std::error_code internal_fixup::end_object () {
    ...
    fixups_->emplace_back (section_, static_cast<pstore::repo::relocation_type> (type_), offset_,
                           addend_);
    return pop ();
  }
  ...
  // name
  // ~~~~~
  gsl::czstring external_fixup::name () const noexcept {
    return "external fixup";
  }
  ...
  std::error_code external_fixup::end_object () {
    ...
    fixups_->emplace_back (*name, static_cast<repo::relocation_type> (type_),
                           is_weak_ ? repo::binding::weak : repo::binding::strong, offset_,
                           addend_);
    return pop ();
  }
