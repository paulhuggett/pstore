...
namespace pstore::exchange::export_ns {
  ...
  ostream_base::ostream_base (std::size_t const buffer_size) {
    buffer_.resize (std::min (
      buffer_size, unsigned_cast (std::numeric_limits<gsl::span<char const>::index_type>::max ())));
    ptr_ = buffer_.data ();
  ...
  ostream_base & ostream_base::write (char const * const s, std::streamsize const length) {
  ...
  void ostringstream::flush_buffer (std::vector<char> const & buffer, std::size_t const size) {
