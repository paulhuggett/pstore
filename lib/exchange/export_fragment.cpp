...
namespace pstore::exchange::export_ns {
  ...
  void emit_fragment (ostream_base & os, indent const ind, class database const & db,
                      ...
                      std::shared_ptr<repo::fragment const> const & fragment, bool const comments) {
    os << "{\n";
    auto const * section_sep = "";
    for (repo::section_kind const section : *fragment) {
      auto const object_indent = ind.next ();
      os << section_sep << object_indent << '"' << emit_section_name (section) << R"(":)";
#define X(a)                                                                                       \
  case repo::section_kind::a:                                                                      \
    emit_section<repo::section_kind::a> (                                                          \
      os, object_indent, db, strings, fragment->at<pstore::repo::section_kind::a> (), comments);   \
    break;
      switch (section) {
