...
namespace pstore::exchange::import_ns {
  ...
  // (ctor)
  // ~~~~~~
  definition::definition (not_null<context *> const ctxt, not_null<container *> const definitions,
...
#undef X
  ...
  // name
  // ~~~~
  gsl::czstring definition_object::name () const noexcept {
  ...
  // end array
  // ~~~~~~~~~
  std::error_code definition_object::end_array () {
    return this->pop ();
  }
  ...
  std::error_code compilation::key (std::string const & k) {
    ...
    if (k == "definitions") {
      ...
      return push_array_rule<definition_object> (this, &definitions_, names_, fragments_);
    }
