...
namespace {
  ...
  void from_8_to_16_traits::convert (input_char_type const * wstr, std::size_t wstr_length,
    ...
    int const wchars_written =
      conversion_result ("MultiByteToWideChar",
                         ::MultiByteToWideChar (
                           CP_UTF8, 0,
                           wstr,                        // input
  ...
  std::size_t from_16_to_8_traits::output_size (input_char_type const * wstr, std::size_t length) {
    return conversion_result (
      "WideCharToMultiByte",
  ...
  namespace utf {
    namespace win32 {
      ...
      std::string to_mbcs (wchar_t const * utf16, std::size_t length) {
        ...
        int size_needed =
          ::WideCharToMultiByte (CP_ACP, 0, &utf16[0], input_length, nullptr, 0, nullptr, nullptr);
        if (size_needed == 0) {
