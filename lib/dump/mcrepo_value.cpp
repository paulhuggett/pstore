...
#undef X
    ...
    value_ptr make_section_value (repo::generic_section const & section,
      ...
      if (!data_value) {
        if (parm.hex_mode) {
          data_value = std::make_shared<binary16> (std::begin (payload), std::end (payload));
        } else {
          data_value = std::make_shared<binary> (std::begin (payload), std::end (payload));
        }
      }
      ...
      return make_value (object::container{
        ...
        {"data", data_value},
        {"ifixups",
         make_value (std::begin (internal_fixups), std::end (internal_fixups),
                     [&] (repo::internal_fixup const & ifx) { return make_value (ifx, parm); })},
        {"xfixups",
         make_value (std::begin (external_fixups), std::end (external_fixups),
                     [&] (repo::external_fixup const & xfx) { return make_value (xfx, parm); })},
      });
    }
    ...
    value_ptr make_section_value (repo::bss_section const & section, repo::section_kind const sk,
  ...
  case repo::section_kind::k:                                                                      \
    ...
    break;
      ...
      for (repo::section_kind const kind : fragment) {
        ...
        array.emplace_back (
          make_value (object::container{{"type", make_value (kind)}, {"contents", contents}}));
      }
...
#undef X
    ...
    value_ptr make_value (index::debug_line_header_index::value_type const & value,
      ...
      return make_value (object::container{
        ...
        {"debug_line_header", make_debuglineheader_value (
                                debug_line_header.get (),
                                debug_line_header.get () + value.second.size, parm.hex_mode)}});
    }
