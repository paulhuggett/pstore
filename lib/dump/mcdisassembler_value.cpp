...
namespace {
  ...
  unsigned address_printer::hex_width (pstore::gsl::span<std::uint8_t const> const & span) {
    ...
    std::size_t const num_bytes =
      (sizeof (usize) * 8U - pstore::bit_count::clz (static_cast<usize> (size)) + 8U - 1U) / 8U;
  ...
  auto create_asm_streamer (llvm::MCContext & context,
    ...
    if (streamer_ptr streamer{
          llvm::createAsmStreamer (context, std::move (formatted_os), false /*is verbose asm*/,
                                   ...
                                   std::unique_ptr<llvm::MCAsmBackend>{}, true /*showInst*/)}) {
  ...
  error_or<bool> emit_instructions (disassembler_state & state,
    ...
    return create_asm_streamer (state.context, std::move (os), state.inst_printer.release ()) >>=
           emit;
  }
...
namespace pstore {
  namespace dump {
    ...
    value_ptr make_disassembled_value (std::uint8_t const * const first,
                                       std::uint8_t const * const last, parameters const & parm) {
