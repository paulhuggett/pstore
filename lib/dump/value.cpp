...
namespace pstore::dump {
  ...
  OStream & string::write_codepoint_hex (OStream & os, UCharType ch) {
    ...
    return os << "\\x" << to_hex<UCharType> ((ch >> 4U) & 0x0FU) << to_hex<UCharType> (ch & 0x0FU);
  }
  ...
  OStream & array::writer (OStream & os, indent const & ind) const {
    ...
    if (is_compact) {
      ...
      for (value_ptr const & value : values_) {
        os << sep << ind << "- ";
        value->write_impl (os, ind.next (value->dynamic_cast_object () == nullptr ? 4 : 2));
        sep = "\n";
      }
    }
    return os;
  ...
  // ~~~~~~~~~~~~~~~
  template <typename OStream, typename ObjectCharacterTraits>
  OStream & object::write_full_size (OStream & os, indent const & ind,
                                     object_strings<ObjectCharacterTraits> const & strings) const {
    ...
    for (auto const & a : kvps) {
      ...
      os << sep << (*std::get<key_index> (a)) << string_type (num_spaces, strings.space_str[0])
         << strings.colon_space_str;
  ...
  value_ptr object::get (std::string const & name) {
    auto const it =
      std::find_if (std::begin (members_), std::end (members_),
  ...
  OStream & binary16::writer (OStream & os, indent const & ind) const {
    ...
    if (v_.empty ()) {
      ...
      for (std::uint8_t const b : v_) {
        ...
        std::array<char_type, 3> const hex{
          {to_hex<char_type> ((b >> 4U) & 0x0FU), to_hex<char_type> (b & 0x0FU), '\0'}};
        os << hex.data ();
        ++byte_no;
      }
    }
