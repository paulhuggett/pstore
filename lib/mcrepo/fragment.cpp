...
bool fragment::fragment_appears_valid (fragment const & f, pstore::extent<fragment> const & fext) {
  ...
  for (auto index_it = std::begin (indices), index_end = std::end (indices); index_it != index_end;
       ++index_it) {
...
std::size_t fragment::size_bytes () const {
  ...
  auto const last_section_size = make_dispatcher (*this, last_section_kind, &buffer)->size_bytes ();
