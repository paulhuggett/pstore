...
namespace pstore {
  namespace repo {
    ...
    std::size_t linked_definitions::size_bytes (std::uint64_t const size) noexcept {
      return sizeof (linked_definitions) - sizeof (definitions_) + sizeof (definitions_[0]) * size;
    }
    ...
    // write
    // ~~~~~
    std::uint8_t * linked_definitions_creation_dispatcher::write (std::uint8_t * const out) const {
